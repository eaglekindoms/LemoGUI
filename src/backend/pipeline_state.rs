...
impl<'a> PipelineState {
    pub fn create_glob_pipeline(globe_state: &'a GlobeState, texture_state: &'a TextureState) -> Self {
        // 纹理渲染配置：纹理着色器，文字着色器，矩形着色器。
        let font_shader = Shader::create_font_shader(globe_state);
        let shape_shader = Shader::create_shape_shader(globe_state);

        // 固定渲染管道配置：纹理管道，矩形管道，边框管道。
        // 全局设置
        let render_pipeline =
            PipelineState::create_pipeline_state(globe_state, &font_shader, RenderType::Texture(&texture_state.texture_bind_group_layout));
        let shape_pipeline =
            PipelineState::create_pipeline_state(globe_state, &shape_shader, RenderType::Shape);
        let border_pipeline =
            PipelineState::create_pipeline_state(globe_state, &shape_shader, RenderType::Border);
        Self {
            render_pipeline,
            shape_pipeline,
            border_pipeline,
